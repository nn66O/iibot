...
off =None
ch = "@nn66a" # يوزر قناتك
DEVS = ["حمودي سورس ", "@nn66a"] #يوزرات المطورين المصنع
...
async def start(client, message):
   if message.from_user.username in DEVS:
     ...
     return await message.reply_text("اهلا بيك ف مصنع بوتات AS ميوزك", reply_markup=kep)
   kep = ReplyKeyboardMarkup([["صنع بوت", "حذف بوت"], ["السورس"]], resize_keyboard=True)
   await message.reply_text("اهلا بيك ف مصنع بوت AS ميوزك", reply_markup=kep)
...
async def alivehi(client: Client, message):
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("حمودي ", url=f"https://t.me/nn66a"),
                InlineKeyboardButton("حمودي ", url=f"https://t.me/nn44u"),
            ],
            [
                 InlineKeyboardButton(f"حمودي سورس ", url=f"https://t.me/nn66y")
            ]
        ]
    )

    await message.reply_photo(
        photo="https://ibb.co/bgxJcYt",
        caption="",
...
async def makedhelal(client, message):
  ...
  if message.from_user.username in DEVS:
    ...
    try:
      ...
      return await message.reply_text("قم بارسال الايدي بشكل صحيح")#'‹ ٰ💸 ⇣ حمودي سورس ⇣ 💸 › .'#
